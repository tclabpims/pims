...
$(function() {
	...
	$("#new").jqGrid({
		...
		colModel: [
			...
			{ name: 'sampathologycode', index: 'sampathologycode',width:'150px', align: "center"},
...
//                    alert(color);
                    var ids = $("#new").getGridParam("reccount");
                    for(var j=1;j<=ids;j++){
	                var rowData = $("#new").jqGrid('getRowData',j);
                    var rowIds = $("#new").jqGrid('getDataIDs');
//                    alert("表状态"+rowData.samsamplestatus);
//                    alert("数据库状态"+state);

                    if(state==rowData.samsamplestatus){
//                    alert(1);
                    $("#new").children().children("tr[id='"+j+"']").children("td").eq(0).css("background-color",color);
                    }
                    }
                }
            }
