...
public class PimsPathologyFeeManagerImpl extends GenericManagerImpl<PimsPathologyFee, Long> implements PimsPathologyFeeManager {
    ...
    @Override
    @Transactional
    public void saveFee(PimsPathologyOrder pathologyOrder, long pathologyId, Set<Long> testItem,List<Long> testItemList) {
        if(testItem.size() > 0) {
            ...
            if(result.size() > 0) {
                ...
                Iterator<Long> it = testItem.iterator();
                for(Long itemid:testItemList){
                    for(Object obj : result) {
                        PimsPathologyFee fee = new PimsPathologyFee();
                        Date dt = new Date();
                        Object[] item = (Object[])obj;
                        if(((BigDecimal)item[4]).longValue() == (itemid.longValue())){
...
//                        fee.setFeeid(((BigDecimal)item[0]).longValue());
                            ...
                            fee.setFeeitemid(((BigDecimal)item[0]).longValue());
                            ...
                            fee.setFeecategory(String.valueOf(pathologyOrder.getOrderid()));
                            save(fee);
                            break;
                        }
                    }
